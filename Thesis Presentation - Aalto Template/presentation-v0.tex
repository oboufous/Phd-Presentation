...
\begin{enumerate}
    ...
    \begin{enumerate}\setlength{\itemsep}{-0.05cm}
    	\item Introduction
        \item Related Work \& Research Questions
...
\begin{figure}[!ht]
  \centering
  \includegraphics[scale=0.35]{"./figures/Aumann_Robert.jpg}
  \caption{Robert John Aumann}
\end{figure}
\end{column}
\end{columns}
...
\begin{itemize}\setlength{\itemsep}{-0.1cm}
    ...
    \item A proof of existence and a generalization to infinite games in (Hart \& Schmeidler, 1989)
    \item Extensive form correlated equilibrium defined in (Von Stengel \& Forges, 2008)   
    \item Other extensions in (Forges, 2020), (Brandenburger \& Dekel, 1992) and (Grant \& Stauber, 2022)
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{Learning Correlated Equilibria}
\begin{itemize}\setlength{\itemsep}{-0.1cm}
    \setlength{\itemindent}{.2in}
    %\item Regret Matching (Hart \& Mas-Colell, 2000) converging to the set of correlated equilibrium distributions 
	\item Regret-matching (Hart \& Mas-Colell, 2000) implying the convergence to the \underline{set} of correlated equilibria 
	\item Correlated Q-learning for selection of specific correlated equilibrium points (Greenwald \& Hall, 2003) 
	\item Convergence to the social welfare maximizing correlated equilibrium (Borowski et al., 2015) 
	\item Learning algorithms for specific applications e.g., resource allocation game (Cigler \& Faltings, 2011)
    %under appropriate assumptions on utilities and the feasible set of joint strategies
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{Constraints \& Correlated Equilibria}
\begin{itemize}\setlength{\itemsep}{-0.1cm}
    \setlength{\itemindent}{.2in}
    \item (Debreu, 1952) defines the concept of generalized equilibrium
    \item (Arrow \& Debreu, 1954) shows a proof of existence
    \item (Rosen, 1965) shows existence and uniqueness of equilibria in concave games with coupled constraints
    \item (Bernasconi et al., 23) introduces the concept of constrained Phi-equilibrium
    %under appropriate assumptions on utilities and the feasible set of joint strategies
\end{itemize}

\textbf{Many Other Theoretical and Applied Contributions}
\begin{itemize}\setlength{\itemsep}{-0.1cm}
    \setlength{\itemindent}{.2in}
    \item \textcolor{red}{XXX} (Debreu, 1952)
    \item \textcolor{red}{XXX} (Arrow \& Debreu, 1954)
\end{itemize}
...
\vspace{-0cm}
\begin{block}{Definition 2 -- Correlated equilibrium distribution (Forges, 2020)}%\textsuperscript{$2$}  
\vspace{-0.1cm}
A correlated equilibrium distribution of $G$ is a probability distribution $\bm{p} \in \Delta(\mathcal{A})$ such that,
\vspace{-0.3cm}
\begin{align}
...
\vspace{0.3cm}
A finite game $G$ is played repeatedly through time $t= 1, 2, 3, ...$
...
\vspace{-0.15cm}
\begin{align}
\scriptstyle  R_{i}^{t}(j, k) = max\Bigg(0, \frac{1}{t} \sum\limits_{\tau \leq t : a_i^\tau = j}\left[u_{i}\left(k, \bm{a}^{\tau}_{-i}\right)-u_{i}\left(\bm{a}^{\tau}\right)\right]\Bigg)
\end{align}
\end{block}
\vspace{-0.2cm}

\textbf{Regret-Matching algorithm (Hart \& Mas-Colell, 2000) applied to a 3-by-2 game}
...
At time $t$, each player $i$ is characterized by a mood $m_i^t \in \{ \textcolor{red}{syn}, \; \textcolor{blue}{asyn} \}$
\vspace{1cm}
...
\column{.5\textwidth}
\begin{figure}[h!]
\hspace{-0.2cm} \scalebox{0.8}{\input{figures/learning/learning2.tex}}
%\caption{Agent $i$ state dynamics}
\end{figure}
\end{columns}
...
\begin{frame}{CPRM algorithm}
\begin{figure}[h!]
\hspace{-0.2cm} \scalebox{0.8}{\input{figures/learning/learning8.tex}}
%\caption{Agent $i$ state dynamics}
\end{figure}
...
\end{columns}
\vspace{0.4cm}
\textbf{Characteristics of The Stochastic Process}
\begin{itemize}
\setlength{\itemindent}{1.8em}
\setlength\itemsep{-0.1em}
\item Transition probabilities depend on the regrets and a perturbation parameter $\varepsilon > 0$ 
\item The stochastic process $\bm{X}^t = (\bm{X}^t_1, ..., \bm{X}^t_n)$ describing players' states is a perturbed non-homogeneous Markov chain 
\item \textcolor{red}{X X X X X X X}
...
% new slide ===========================================================
\begin{frame}[t]
\frametitle{An application to a congestion game}
...
\begin{block}{Definition 1 -- Correlated equilibrium (Forges, 2020)}
\vspace{-0.1cm}
A correlated equilibrium of $G$ is a pair $(d, \bm{\alpha}^*)$ where $\bm{\alpha}^* : \Omega \rightarrow \mathcal{A}$ is a correlated strategy profile such that  
\vspace{-0.2cm}
\begin{align}
...
\setlength{\itemindent}{1em}
    ...
    \end{align}
    \item Performance measures in many applications can be expressed in terms of probability distribution  over action profiles. Examples include applications in smart grids, wireless networks, etc. 
    \begin{itemize}
    \setlength\itemsep{-0.2em}
    \setlength{\itemindent}{2em}
    \item  Constraints on the social welfare : $\sum_{i \in \mathcal{N}}\sum_{\bm{a} \in \mathcal{A}} \bm{p}(\bm{a})u_i(\bm{a}) \geq D_1$ 
...
\begin{itemize}\setlength{\itemsep}{-0.1cm}
    \setlength{\itemindent}{1em}
    \item The set of feasible strategies is the set of pure action profiles 
    \item The players must play a correlated strategy profile inducing a pure action profile in $G$. 
    \item There does not exist a constrained correlated equilibrium for this game 
\end{itemize}
%%%%%%%%%%%%%%%%%
\vspace{0.3cm}
...
Let $\mathcal{C} \subseteq \Delta(\mathcal{A})$ be a set of probability distributions, called {feasible set of probability distributions} and for each correlation device $d$, define the coupled constraint set $\mathcal{R}_d$ generated by $\mathcal{C}$ such that, 
%%%%%%%%%%%%%%%%%%%
\begin{equation}\label{eq:RdInducedByFeasibleDistributions}
    \mathcal{R}_d = \{  \bm{\alpha}  \in \mathcal{S}_d \mid \bm{p}_{\bm{\alpha}} \in \mathcal{C} \}
\end{equation}
%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textcolor{aaltoblue}{\large Theorem 1} :
    Let $G$ be a finite non-cooperative game and $\mathcal{C}$ a set of feasible probability distributions. 
    The distribution $\bm{p}\in\Delta(\mathcal{A})$ is a constrained correlated equilibrium distribution 
    if and only if 
    for any player $i\in\mathcal{N}$,
    ...
    \begin{align}
        ...
        \right] 
        \geq 0 
        \label{eq:defnCanonical}
    \end{align}
    %%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{-0.3cm}

$\rightarrow$ The set of constrained correlated equilibrium distributions is obtained using the class of canonical correlation devices.

\vspace{0.5cm}

Define the mixed extension $\Delta G$, 
...
\begin{figure}
      ...
      \caption{Utilities - Constraint: Social Welfare $\geq 12$}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{column}
